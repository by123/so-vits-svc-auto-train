...
def main(max_steps):
    ...
    mp.spawn(run, nprocs=n_gpus, args=(n_gpus, hps, max_steps))


def run(rank, n_gpus, hps, max_steps):
    global global_step
    if rank == 0:
    ...
    for epoch in range(epoch_str, hps.train.epochs + 1):
        ...
        if rank == 0:
            train_and_evaluate(rank, epoch, hps, [net_g, net_d], [optim_g, optim_d], [scheduler_g, scheduler_d], scaler,
                               [train_loader, eval_loader], logger, [writer, writer_eval], max_steps)
        else:
            train_and_evaluate(rank, epoch, hps, [net_g, net_d], [optim_g, optim_d], [scheduler_g, scheduler_d], scaler,
                               [train_loader, None], None, None, max_steps)
        # update learning rate
        scheduler_g.step()
        scheduler_d.step()


def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers, max_steps):
    net_g, net_d = nets
...
if __name__ == "__main__":
    ...
    parser.add_argument('--step', '-step', type = str, default = '3000' , help = '训练最大的步数，用字符串类型')
